...
- 利用条件: 記事本文とパッケージに明示の記載なし。同シリーズの前版では、2026-06-10 版が「利用・改変は自由ですが改変無しの再配布は御遠慮頂けると嬉しいです」、2026-03-11 版が「改変含めご自由に。無料であれば二次配布可」と記している。**public リポジトリへの同梱について作者に確認中**
- こちらでの扱い: `templates/novel/` に Claude Code 版を原文のまま置き、Codex CLI 版から `.agents/`、`.codex/`、`AGENTS.md` を加えた。ファイルの中身は変えていない。**許諾が出るまで `templates/novel/` はリポジトリに含めていない**（`.gitignore`）。`templates/novel.json` は出典と初期化手順だけを持つ
- 関連記事:
