...
このリポジトリは、三人称多元視点の日本語長編Web小説を、数十万字を超えても設定・時系列・人物像・伏線を破綻させずに執筆するための環境である。Codex はセッション開始時に本ファイルをリポジトリ指示として読み込む。詳細な本文規約は `guidelines/`、再利用可能な手順は `.agents/skills/` に置く。競合時は、ユーザーの明示指示、本ファイル、対象ガイドライン、対象スキルの順で従う。セットアップと運用の詳細が必要なときは `MANUAL.md` を参照する。
...
```text
AGENTS.md            # Codexが自動で読む最優先規約（本ファイル）
README.md            # 導入手順（利用者向け）
MANUAL.md            # 取り扱いマニュアル（最初に読む）
.agents/skills/      # 執筆・診断・相談ワークフロー
.codex/config.toml   # プロジェクト設定（sandbox・approval・web search）
...
- 主要ワークフローは自然文またはSkillの明示指定（`$long-novel-orchestrator`、`$validate-episode`、`$novel-plot-advisor`、`$novel-worldbuilding-advisor`）で起動する。
...
シェル操作は、Codexのサンドボックスと承認ポリシーを保ったまま、意図しない上書きや承認待ちの連鎖を避ける。

- ファイルの作成・追記・変更は `apply_patch` などのファイル編集機能を使う。シェルのリダイレクト書込（`>`、`>>`）やヒアドキュメントで代用しない。
- 検索は `rg` と `rg --files` を優先する。本文・資料は必要な範囲だけ読み、巨大なファイルを無差別に出力しない。
- 読み取りと変更を一つの複合コマンドへ混ぜない。破壊的操作は対象の絶対パスを確認し、ユーザーの依頼範囲内かを確かめる。
- 終了コードの確認に別のコマンドを足さず、実行結果から判定する。
- Bashスクリプトは Bash で、Pythonスクリプトは Python 3.11以降で実行する。WindowsネイティブでGNU grepがない場合はWSLまたはGit Bashを使う。
